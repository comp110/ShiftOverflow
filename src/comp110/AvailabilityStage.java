...
public class AvailabilityStage extends KarenStage {
	...
	public AvailabilityStage(String title, Employee currentEmployee, Controller controller, UI ui) {
	...
	public void renderAvailabilityStage(Employee employee) {
	...
	private void renderAvailabilityStage() {
		...
		if (_currentEmployee != null) {
			_onyenField = new TextField(_currentEmployee.getOnyen());
		} else {
		...
		_performSwapButton.setOnAction((event) -> {
			...
			if (_ui.getSchedule() == null) {
				_controller
						.displayMessage("There is no schedule loaded yet. A schedule must be loaded before swapping.");
				return;
			} else {
				// let them swap
				new PerformSwapStage("Perform Swap", _controller, _currentEmployee, _ui);
			}
		...
		if (_currentEmployee != null) {
			nameField.setText(_currentEmployee.getName());
		} else {
			nameField.setText("Name");
		}
		nameField.textProperty().addListener((observable, oldValue, newValue) -> {
			if (_currentEmployee != null) {
				_currentEmployee.setName(newValue);
			}
		});
		ComboBox<String> genderDropdown = new ComboBox<String>();
		if (_currentEmployee != null) {
			genderDropdown.getSelectionModel().select(_currentEmployee.getIsFemale() ? "Female" : "Male");
		}
		genderDropdown.getItems().addAll("Male", "Female");
		genderDropdown.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
			if (_currentEmployee != null) {
		...
		if (_currentEmployee != null) {
			...
			// indexed
			capacityDropdown.getSelectionModel().select(_currentEmployee.getCapacity() - 1);
		}
		capacityDropdown.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
			if (_currentEmployee != null) {
		...
		if (_currentEmployee != null) {
			// have to -1 because it is pulling by index and list is zero
			// indexed
			levelDropdown.getSelectionModel().select(_currentEmployee.getLevel() - 1);
		}
		...
		for (int day = 0; day < 8; day++) {
			...
			for (int hour = 0; hour < 12; hour++) {
				...
				if (day == 0) {
					...
					if (_currentEmployee != null) {
						if (_currentEmployee.isAvailable(day - 1, hour + 9)) {
